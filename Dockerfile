...
ARG DS_REGION=Dev
ARG DS_TZ=UTC
RUN mkdir -p out/etc/nginx/ds/server-includes \
 && for f in nginx/server-includes/*.j2; do \
      jinja2 "$f" \
        -D ds_node_id="$DS_NODE_ID" \
        -D ds_city="$DS_CITY" \
        -D ds_region="$DS_REGION" \
        -D ds_tz="$DS_TZ" > "out/etc/nginx/ds/server-includes/$(basename "${f%.j2}")"; \
    done \
 && cp nginx/server-includes/20-ds-api.conf out/etc/nginx/ds/server-includes/ \
 && mkdir -p out/etc/nginx/conf.d out/etc/nginx/snippets /usr/share/nginx/html \
